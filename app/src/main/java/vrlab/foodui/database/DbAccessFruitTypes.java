...
public class DbAccessFruitTypes extends SQLiteOpenHelper {
    ...
    public List<FruitType> getAllData(){
        ...
        for(int i = 0; i < count; i++) {
            ...
            String nutritionalValueVectorString;
            String nutritionalValueVectorHealthHazard;
            ...
            try {
                nutritionalValueVectorString = cursor.getString(12);
               // nutritionalValueVector = convertStringToFloatArray(cursor.getString(12));
               // nutritionalValueVectorHealthHazard = nutritionalValueVectorString;
                String[] strings = nutritionalValueVectorString.replace("[", "").replace("]", "").split(", ");
                float result[] = new float[strings.length];
                for (i = 0; i < result.length ; i++) {
                    result[i] = Float.parseFloat(strings[i]);
                }

                nutritionalValueVectorHealthHazard =  cursor.getString(13);

                Log.d("ddddddddddddddddddddd" , nutritionalValueVectorHealthHazard);

                nutritionalValue = new NutritionalValue(result[0],
                                        result[1], result[2],
                                        result[3], result[4] , nutritionalValueVectorHealthHazard);
            } catch(Exception e) {
                Log.e(TAG, e.getMessage());
            }
    ...
    public void add(FruitType ft){
        ...
        String nutritionValueHealthHazard;
        ...
        if (ft.getNutritionalValue()!=null) {
            ...
            nutritionValueHealthHazard = ft.getNutritionalValue().getHealthHazardInformation();
            data.put("nutritionalValue", Arrays.toString(nutritionalValue));
            data.put("HealthHazard" , nutritionValueHealthHazard);
        }
